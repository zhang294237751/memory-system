"""
SessionEnd Hook 脚本
在每次会话结束时自动保存对话摘要到 .claude/session_abstract.md
"""
import json
import sys
import os
...
def main():
    cwd = os.getcwd()
    debug_path = os.path.join(cwd, ".claude", "hooks", "debug.log")
    raw_path = os.path.join(cwd, ".claude", "hooks", "raw_input.txt")
    ...
    try:
        ...
        output_path = os.path.join(cwd, ".claude", "session_abstract.md")
